        ...
        <table id="tableData" class="table align-middle border-bottom">
            ...
            @foreach($products as $item)
                <tr>
                    ...
                    <td>
                        <button id={{"button_buy_price_".$item->id}} onclick="open_buy_page({{ $item->id }},'buy')" class="btn btn-success btn-status" @if($market=="closed" || $item->buy_status=='0') disabled @endif >خرید</button>
                        <button id={{"button_sell_price_".$item->id}} onclick="open_buy_page({{ $item->id }},'sell')" class="btn btn-danger btn-status" @if($market=="closed" || $item->sell_status=='0') disabled @endif  >فروش</button>
