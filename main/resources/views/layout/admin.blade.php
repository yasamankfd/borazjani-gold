...
<!-- ------------------sidebar------------------- -->
<div x-data="{ toggle:true }" :class="!toggle ? 'w-20' : 'w-72' " class="hidden sm:block navigation text-white bg-colorprimary py-5 overflow-y-auto">
    ...
    <ul x-data="{dashboard:true, information:false, managemarket:false, orders:false, transaction:false, customer:false,users:false, setting:false, profile:false}" class="mt-5 space-y-3 relative font-light">
        <li :class="dashboard ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("dashboard-admin") }}"  @click="dashboard=true, information=false, managemarket=false, orders=false, transaction=false, customer=false,users=false, setting=false, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
        ...
        <li :class="information ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("admin-products") }}" @click="dashboard=false, information=true, managemarket=false, orders=false, transaction=false, customer=false,users=false, setting=false, profile=false" class="cursor-pointer flex items-center justify-between items-center relative w-full p-2 rounded-r-full">
                <span class="flex gap-7">
                    ...
                    <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">اطلاعات پایه</span>
                </span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" :class="information ? 'rotate-0' : 'rotate-90'"  class="w-4 h-4">
        ...
        <li :class="managemarket ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("market-change-page")}}" @click="dashboard=false, information=false, managemarket=true, orders=false, transaction=false, customer=false,users=false, setting=false, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                <span class="icon relative block text-center">
                    <img src="{{ url("/img/managemarket-icon.svg")}}" alt="" class="">
                </span>
                <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">مدیریت بازار</span>
            </a>
        </li>
        <li :class="orders ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("admin-liveorders") }}" @click="dashboard=false, information=false, managemarket=false, orders=true, transaction=false, customer=false,users=false, setting=false, profile=false" class="relative cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                <span class="icon relative block text-center">
        ...
        <li :class="transaction ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("admin-transactions") }}" @click="dashboard=false, information=false, managemarket=false, orders=false, transaction=true, customer=false,users=false, setting=false, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                <span class="icon relative block text-center">
                    <img src="{{ url("/img/transaction-icon.svg")}}" alt="" class="">
                </span>
                <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">گزارش معاملات</span>
            </a>
        </li>
        <li :class="customer ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("admin-manage-customers") }}" @click="dashboard=false, information=false, managemarket=false, orders=false, transaction=false, customer=true,users=false, setting=false, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                ...
                <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">مشتریان</span>
            </a>
        </li>
        <li :class="users ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a href="{{ route("admin-manage-users") }}" @click="dashboard=false, information=false, managemarket=false, orders=false, transaction=false, customer=false,users=true, setting=false, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                <span class="icon relative block text-center">
                    <img src="{{ url("/img/users-icon.svg")}}" alt="" class="">
                </span>
                <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">کاربران</span>
            </a>
        </li>
        <li :class="setting ? 'bg-colorsecondry1' : 'bg-transparent'" class="pr-5 relative group">
            <a  @click="dashboard=false, information=false, managemarket=false, orders=false, transaction=false, customer=false,users=false, setting=true, profile=false" class="cursor-pointer flex justify-start gap-7 items-center relative w-full p-2 rounded-r-full">
                <span class="icon relative block text-center">
                    <img src="{{ url("/img/setting-icon.svg")}}" alt="" class="">
                </span>
                <span x-show="toggle" x-transition.duration.300ms class="relative block text-start whitespace-nowrap font-yekan text-sm hover:scale-105 transition-all">تنظیمات</span>
            </a>
        </li>
    ...
    <div x-on:click="toggle = !toggle" class="toggle absolute top-4 left-2 bg-white rounded-full cursor-pointer p-2 flex justify-center items-center">
        <svg :class="!toggle ? 'rotate-180' : 'rotate-0' " xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-colorprimary transition-all">
...
<div class="fixed w-full bottom-0 sm:hidden z-20">
    ...
    <div class="bg-colorprimary px-4 flex justify-around items-center">
        ...
        <a href="" class="flex flex-col items-center text-white gap-2 px-4 py-2">
            <img src="{{ url("/img/managemarket-icon.svg")}}" alt="">
            <span class="text-xs">بازار</span>
        </a>
    </div>
...
<!-- فیلتر  -->
<div class="navbar-menu w-full h-screen bg-colorprimary/70 backdrop-blur-md text-white fixed top-0 z-50 font-alibaba p-5 flex md:hidden flex-col gap-5 overflow-y-auto">
    ...
    <a href="#"  class="flex gap-2 items-center">
        <img src="{{ url("/img/information-icon.svg")}}" alt="">
        <span class="font-light text-sm">اطلاعات پایه</span>
    </a>


    <a href="#"  class="flex gap-2 items-center">
        <img src="{{ url("/img/managemarket-icon.svg")}}" alt="">
        <span class="font-light text-sm">مدیریت بازار</span>
    </a>
    ...
    <a href="#"  class="flex gap-2 items-center">
        ...
        <span class="font-light text-sm">گزارشات معاملات</span>
    </a>

    <a href="#"  class="flex gap-2 items-center">
        <img src="{{ url("/img/customers-icon.svg")}}" alt="">
        <span class="font-light text-sm">مشتریان</span>
    </a>

    <a href="#"  class="flex gap-2 items-center">
        <img src="{{ url("/img/users-icon.svg")}}" alt="">
        <span class="font-light text-sm">کاربران</span>
    </a>

    <a href="#"  class="flex gap-2 items-center">
        <img src="{{ url("/img/setting-icon.svg")}}" alt="">
        <span class="font-light text-sm">تنظیمات</span>
    </a>
...
<div dir="ltr" class=" w-full h-full sm:h-screen px-0 sm:px-5  sm:py-5">
    ...
    <div dir="rtl" class="bg-gray-100 rounded-none sm:rounded-3xl main h-full py-5 px-5 overflow-y-auto scrollbar">
        ...
        <div class="mt-5">
            <!-- ------------------- نمایش وضعیت بازار -----------------------  -->
            <div class="w-full flex flex-col justify-center items-center border-b pb-3">
                <span class="font-light">وضعیت بازار:</span>
                <div>

                    <label class="flex gap-2 items-center cursor-pointer">
                        <span class="text-sm font-medium text-gray-900 dark:text-gray-300">بسته</span>
                        <input name="market_status" id="market_status" type="checkbox" @if($market=="open") checked @endif onclick="change_market_status()" value="" class="sr-only peer">
