...
<div class="container mt-5 mb-5">
    ...
    <form action="{{ route('admin') }}" method="post">
        <div class="table-container">
            ...
            @foreach($products as $item)
                ...
                <div class="table-cell">{{ $item->title }}</div>
                    ...
                    <div class="table-cell">
                        ...
                        <div>
                            <label>قابلیت خرید</label>
                            <label class="switch mb-1">
                                <input name="product_buy_status_{{ $item->id }}" id="product_buy_status_{{ $item->id }}" type="checkbox" @if($item->buy_status=='1') checked @endif onclick="change_product_status({{ $item->id }},'buy')">
                                <span class="slider round"></span>
                            </label>
                            <label>قابلیت فروش</label>
                            <label class="switch mb-1">
                                <input name="product_sell_status_{{ $item->id }}" id="product_sell_status_{{ $item->id }}" type="checkbox" @if($item->sell_status=='1') checked @endif onclick="change_product_status({{ $item->id }},'sell')">
                                <span class="slider round"></span>
                            </label>
                        </div>
                    </div>
            ...
            <button onclick="window.open('{{ route('admin-orders') }}')" class="btn btn-dark">Orders</button>
